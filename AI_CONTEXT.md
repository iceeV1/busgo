...
## งานที่ทำแล้ว (v2.0.0 — Next-Gen Smart Transit UI + Live Radar System)

45. **ระบบเรดาร์ติดตามรอบรถสด 24 ชม. (Live Radar & FIDS Telemetry System)**:
    - เพิ่มแท็บ "เรดาร์ติดตามสด" (FIDS Board): แสดงสถานะรถบัสทุกคันแบบเรียลไทม์ พร้อมตัวนับสถานะ (กำลังวิ่งบนถนน / รอออกเดินทาง / ถึงปลายทางแล้ว)
    - คำนวณ Telemetry สด: พิกัดจุดแวะพักปัจจุบัน (Waypoint), จุดจอดถัดไป, ความเร็วจำลองตามจริง (กม./ชม.), แถบเปอร์เซ็นต์ความคืบหน้า (Progress Bar) และเวลานับถอยหลัง ETA
    - โมดัลไทม์ไลน์การเดินทางสด (Journey Timeline Modal): แสดงผังการเดินทางแบบ Station-by-Station ตามเส้นทาง พร้อมสถานะจุดที่ผ่านแล้ว/กำลังผ่าน/ยังไม่ถึง พร้อมข้อมูลคนขับ ทะเบียนรถ และสิ่งอำนวยความสะดวก
    - ปุ่ม "เรดาร์สด" บนทุกการ์ดเที่ยวรถ และบนตั๋วในหน้า "ตั๋วของฉัน" ลิงก์ตรงสู่ข้อมูลเรดาร์ของรถคันนั้นทันที
46. **ยกเครื่องดีไซน์ Next-Gen Luxury Transit (Cyber Obsidian & Warm Pearl)**:
    - Typography: ผสาน Google Fonts (Plus Jakarta Sans, Kanit ภาษาไทย, JetBrains Mono) ให้ความรู้สึกระดับสากล
    - แถบสถานะระบบสด (Live System Ticker): แสดงสถานะดาวเทียม GPS, นาฬิกาดิจิทัลเรียลไทม์ และสถิติความตรงเวลา
    - ผังห้องโดยสารเสมือนจริง (Interactive Cabin Seat Map): โซนคนขับพร้อมพวงมาลัย ประตูทางขึ้น-ลง แสงไฟทางเดิน และการ์ดสรุปยอดเงินสดใส
    - Boarding Pass e-Ticket: ตั๋วโดยสารทรงพรีเมียมพร้อมรอยฉลุ (Tear Notches), Barcode และ QR Code แสดงผลชัดเจน
...
## งานที่ทำแล้ว (v2.1.0 — 3D Holographic GPS Radar Engine)

48. **ระบบแผนที่ 3D โฮโลกราฟิกติดตามรถสด (3D Holographic GPS Command Center)**:
    - **เอนจินการแสดงผล 3D Perspective**: ควบคุมมุมมองระนาบแผนที่ 3 มิติ (`rotateX`, `rotateZ`, `scale`) พร้อมชั้นบรรยากาศ Ionosphere Horizon Glow และเส้นสแกนตารางพิกัดไซเบอร์ (Holographic Scan Grid)
    - **โมเดลรถ 3D แบบ Upright Billboarding**: ตัวรถยืนตรงในมิติ 3D ไม่แบนราบไปกับพื้น พร้อมจานเรดาร์หมุนสแกนบนผิวถนน (Rotating Ground Radar Disc), เสาลำแสงโฮโลแกรมบอกระดับความสูง (Holographic Altitude Pillar), ลำแสงไฟหน้ารถส่องนำทางบนผิวทางหลวง (Forward Headlight Projection) และชิปแสดงความเร็วลอยตัว
    - **เสาสัญญาณสถานี 3D (Waypoint Beacons)**: สถานีหลักทั่วประเทศแสดงเป็นเสาพลังงานแสงสีทองเรืองรอง พร้อมชื่อเมืองลอยตัวในมุมมอง 3 มิติ
    - **แผงควบคุมกล้อง 3D HUD แบบเรียลไทม์**:
      - ปุ่มสลับโหมด 3D โฮโลกราฟิก / 2D แผนที่ปกติ (ไม่มี emoji ตามกติกา)
      - ปุ่มตั้งค่ามุมกล้องด่วน: 3D ไอโซเมตริก (50°), 3D ซีเนมาติก (60°), 2D หน้าตรง (0°)
      - สไลเดอร์ปรับระดับความเอียง (Pitch 0°-65°) และการหมุนทิศ (Bearing -45° ถึง +45°) แบบอินเทอร์แอคทีฟ
      - ระบบหมุนกล้องรอบตัวอัตโนมัติ (Cinematic Auto-Orbit Sweep)
...
## งานที่ทำแล้ว (v2.0.2 — ปรับกลับเป็นแผนที่ 2D เรดาร์สดคมชัดมาตรฐาน)

50. **ยกเลิกมุมมอง 3D ตามคำขอผู้ใช้ คืนสู่แผนที่ 2D เรดาร์สด (Clean 2D Live GPS Map Restored)**:
    - นำระนาบมุมมองเอียง 3D, แผงควบคุมกล้อง HUD, สไลเดอร์ความเอียง/หมุนทิศ และโอเวอร์เลย์ 3D ออกทั้งหมด
    - คืนค่าแผนที่อินเทอร์แอคทีฟ 2D มาตรฐาน คมชัด อ่านง่าย ลื่นไหล สบายตา
    - คงระบบเรดาร์ติดตามพิกัดรถสด 24 ชม., การคำนวณตำแหน่งบนทางหลวงแบบเรียลไทม์ (Waypoint Interpolation), แถบสถานะระบบสด (HUD), และลิ้นชักเลือกดูเที่ยวรถไว้อย่างครบถ้วน 100%
    - **ตั้งค่า Semantic Version เป็น 2.0.2** และอัปเดต Cache-busting `?v=2.0.2` ทั้งหมด

## งานที่ทำแล้ว (v2.2.0 — Multi-Style HD Satellite Hybrid & Cyber Radar Map Switcher)

51. **ยกระดับความสวยงามของแผนที่สู่ระดับพรีเมียม (Next-Gen Map Aesthetics & Style Switcher)**:
    - **ระบบสลับสไตล์แผนที่แบบ 1 คลิก (Map Style Switcher)**:
      - **ดาวเทียมไฮบริด (Satellite Hybrid - ค่าเริ่มต้น)**: ผสานภาพถ่ายดาวเทียมความคมชัดสูงระดับโลกจาก Esri World Imagery ร่วมกับเลเยอร์ป้ายชื่อถนน เมือง และทางหลวงภาษาไทย/สากลความคมชัดสูงจาก Carto Voyager Labels มองเห็นภูมิประเทศ เทือกเขา ทะเล ป่าไม้ และผังเมืองเสมือนจริงสมบูรณ์แบบ
      - **ดาร์กนีออน (Cyber Dark)**: สไตล์มืดแบบเรดาร์ควบคุมการบินและคมนาคมไฮเทค ใช้ไทล์ Carto Dark Matter สีดาร์กออบซิเดียน ตัดกับเส้นทางเดินรถนีออนสว่างไสว
      - **สว่างคลีน (Clean Voyager)**: สไตล์โมเดิร์นคลีน สว่างตา เหมาะสำหรับดูเวลากลางวัน จาก Carto Voyager
    - **ความปลอดภัยและ CSP**: ขยาย Content-Security-Policy ใน `server.js` ให้ครอบคลุม `https://server.arcgisonline.com` และ `https://*.arcgisonline.com` ทั้ง `img-src` และ `connect-src`
    - **การแสดงผลเส้นทางทางหลวงแบบ Glow & Double-layer**: ซ้อนเลเยอร์ Halo แสงสีฟ้ารอบเส้นทาง พร้อมเส้นนีออนประตรงแกนกลาง ให้ความรู้สึกแบบ High-tech Highway Corridor
    - **หมุดรถบัสตามทิศทางการวิ่ง (Aerodynamic Fleet Pins)**: ปรับปรุงหัวลูกศรทิศทาง พร้อม Speed Badge แสดงความเร็วแบบเรียลไทม์ (เช่น `85k`) บนตัวรถทันทีโดยไม่ต้องคลิก
    - **การจดจำการตั้งค่า (Persistent Preference)**: จดจำสไตล์แผนที่ที่เลือกผ่าน `localStorage.busgo_map_style`
    - **มาตรฐานความปลอดภัยและกติกา**: ปราศจาก emoji 100% (ใช้อินไลน์ SVG ทั้งหมด), ผ่าน `node --check`
    - **Bump Semantic Versioning เป็น 2.2.0** และอัปเดต Cache-busting `?v=2.2.0` ทั้งหมด

## งานที่ทำแล้ว (v2.3.0 — Real Road-Geometry Highway Corridors & Road-Following Bus Navigation)

52. **ปรับปรุงเส้นทางเดินรถและทางหลวงให้วิ่งตามแนวถนนจริง 100% (Road-Accurate Highway Corridors & Real-Road Bus Tracking)**:
    - **ชุดข้อมูลพิกัดทางหลวงจริงความแม่นยำสูง (Real Road Geometries Dataset)**: สกัดพิกัดตามแนวถนนจริง (ถนนพหลโยธิน AH2/ทล.1, ถนนมิตรภาพ ทล.2, ถนนพระราม 2/เพชรเกษม ทล.4, ทางหลวงมอเตอร์เวย์ ทล.7, ทางหลวงสาย 1095 ดอยแม่ฮ่องสอน 762 โค้ง) ทั้งหมด 9 เส้นทางหลัก บรรจุใน `routes.js`
    - **การวาดเส้นทางตามแนวถนน (Road-Hugging Polylines)**: แผนที่วาดเส้นทาง Double-layer วิ่งเลี้ยวคดเคี้ยวตามโค้งถนนจริง ไม่ตัดข้ามทุ่งนาหรือภูเขาแบบเส้นตรงอีกต่อไป
    - **ระบบคำนวณตำแหน่งรถบนถนนจริง (Road Interpolation & Adaptive Bearing)**:
      - ฟังก์ชัน `getRoadPath(from, to)` ดึงแนวพิกัดถนนสำหรับแต่ละเที่ยวรถ รองรับทั้งขาไปและขากลับ
      - ฟังก์ชัน `getBusCoordinates(bus, tele)` เคลื่อนที่รถตามจุดพิกัดถนนจริงทีละช่วงอย่างแม่นยำ พร้อมคำนวณมุมหันหน้ารถ (Bearing) ให้เอียงเลี้ยวตามทางโค้งของถนนอัตโนมัติ
